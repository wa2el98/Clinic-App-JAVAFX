...
public class DecryptAllController {
    ...
    @FXML
    void Decrypt(ActionEvent event) throws IOException, ClassNotFoundException, SQLException {
		...
		if (txtDirectory.getText().equals("")) {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				if (extension.equals("txt")) {
					...
					String decrypted_text = Decryption.decrypt(myKey, info);

					// writing back to file
					FileSystem fileSystem = FileSystem.getInstance();
					fileSystem.insertInFile(path, decrypted_text);
				}
